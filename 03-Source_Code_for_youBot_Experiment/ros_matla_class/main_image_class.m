...
img_c = Dig_Img (img,result_Frame(1), result_Frame(2), result_Frame(4), result_Frame(4),  result_Frame(5));
cnn_class = cnn_feedforward(cnn_class, img_c);
n = numel(cnn_class.full_layer);%全连接层的层数
b = cnn_class.full_layer{n}.o;%网络输出
[~, h] = max(b); % 找到最大的输出对应的标签 ://1是黄色，2是粉红，3是绿色，4是蓝色，5是紫色，6是红色
