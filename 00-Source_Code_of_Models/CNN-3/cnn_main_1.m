...
cnn1.conv_layer = {  
    ...
    struct('type', 's', 'scale_1', 2,'scale_2', 3) %空间金字塔池化层
};  
cnn1.full_layer = {  
    struct('neurons', 1)%全连接层，输出层——由train_y决定大小
}; 
cnn1.Parameter.rate = 0.001;%学习率  
cnn1.Parameter.num = 10;%训练num个样本后进行一次权值调整
cnn1.Parameter.train_num = 100000;%整个样本的训练最大次数
cnn1.Parameter.r=0.0001;%训练中的最小误差，当小于r是，停止训练
cnn1.e_num = 0; %出现loss连续不变或增加的计数
