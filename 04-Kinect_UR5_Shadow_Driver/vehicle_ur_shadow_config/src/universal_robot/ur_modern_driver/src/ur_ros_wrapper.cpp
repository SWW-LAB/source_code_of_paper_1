...
#include <ur_modern_driver/SpeedScaleConfig.h>
...
protected:
    ...
    dynamic_reconfigure::Server<ur_modern_driver::SpeedScaleConfig> dynamic_reconfigure_server;
    double speed_scale;
...
public:
	RosWrapper(std::string host, int reverse_port) :
        ...
        dynamic_reconfigure_server.setCallback(boost::bind(&RosWrapper::dynamicReconfigureCallback, this, _1, _2));
...
private:
    void dynamicReconfigureCallback(ur_modern_driver::SpeedScaleConfig &config, uint32_t level) {
        ROS_INFO("Set speed to: %f", config.double_param);
        speed_scale = config.double_param;
    }
		...
		for (unsigned int i = 0; i < traj.points.size(); i++) {
			...
			for (unsigned int j = 0; j < traj.points[i].positions.size(); j++) {
				...
				new_point.velocities.push_back(
						traj.points[i].velocities[mapping[j]]*speed_scale);
				if (traj.points[i].accelerations.size() != 0)
					new_point.accelerations.push_back(
							traj.points[i].accelerations[mapping[j]]*speed_scale*speed_scale);
			}
			new_point.time_from_start = ros::Duration(traj.points[i].time_from_start.toSec() / speed_scale);
