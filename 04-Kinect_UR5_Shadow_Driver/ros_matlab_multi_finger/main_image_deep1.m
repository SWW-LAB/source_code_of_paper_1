...
deep_0_1 = Normalize1(deep_data_image);%归一化
...
x1 = points_data_image(result_Frame(1),result_Frame(2),1);  %或者直接调整物体的位置误差,reult_frame是结果,1是x,2是y,3\4是边框大小,5是角度,6是打分
y1 = points_data_image(result_Frame(1),result_Frame(2),2);
z1 = points_data_image(result_Frame(1),result_Frame(2),3);

x2 = points_data_image(result_Frame(1)+15,result_Frame(2),1);%手掌再根据物体倾斜旋转
...
sin_xi = 0;
cos_xi =1;
%再将坐标转换到世界坐标系中，成为UR5的末端执行器坐标（之后考虑直接作为shadow手的坐标）
...
target_word = camera2word*[x1;y1;z1;1]; %这个直接是shadow的手掌位置
...
wTh = [R2*R1 target_word(1:3)-[0.09;0.0;0.0];%修改shadow手的手掌误差，而不是直接修改UR5的末端位置误差
    0 0 0 1];
% wTh = [R2*R1 target_word(1:3)-[0.09;0.03;0.05];%修改shadow手的手掌误差，而不是直接修改UR5的末端位置误差
%     0 0 0 1];
