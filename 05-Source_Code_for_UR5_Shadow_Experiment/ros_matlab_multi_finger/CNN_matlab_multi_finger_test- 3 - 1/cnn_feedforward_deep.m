function net = cnn_feedforward_deep(net, x)  
    n = numel(net.conv_layer); % 层数
    net.conv_layer{1}.a{1} = x; % 网络的第一层就是输入，但这里的输入包含了多个训练图像 
    ...
    for l_n = 2 : n   %  for each layer  
        if strcmp(net.conv_layer{l_n}.type, 'c') % 卷积层  
            ...
            for j = 1 : net.conv_layer{l_n}.outputmaps   %  for each output map  
               ...
               % z = zeros(size(net.conv_layer{l_n - 1}.a{1}) - [net.conv_layer{l_n}.kernelsize - 1 net.conv_layer{l_n}.kernelsize - 1 0]);   
